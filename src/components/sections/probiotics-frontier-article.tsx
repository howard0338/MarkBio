  ...
  return (
    <div className={cn(siteCopy, "w-full")}>
          ...
          >
            <section aria-labelledby={`frontier-theme-${ti}`}>
              ...
              {theme.bullets && theme.bullets.length > 0 ? (
                <ul className="mt-4 list-disc space-y-2.5 pl-5 marker:text-brand-electric/80">
              ...
              {theme.subBlocks?.map((block) => (
                <div key={block.title} className="mt-8">
                  ...
                  {block.intro ? <p className="mt-3">{block.intro}</p> : null}
                  <ul className="mt-3 list-disc space-y-2 pl-5 marker:text-brand-electric/80">
              ...
              {theme.footnote ? (
                <p className="mt-4 text-brand-navy/70">{theme.footnote}</p>
              ) : null}
            </section>
          </li>
        ))}
      ...
      >
        ...
        <ol className="mt-4 list-decimal space-y-2.5 pl-5 text-xs text-brand-navy/70 sm:text-sm">
